...
for (element in element_list) {
  ...
  for (file in nc_files) {
    ...
    # Try extracting the zip file, catching any errors
    ...
    if (!unzip_status) {
      print(paste("Error extracting:", file))  # Print the file causing the issue
      next  # Skip further processing for this file
    }
    ...
    if (is.na(nc_file)) {
      print(paste("No valid NetCDF file found for:", date_str))
      next
    }
...
ppt_data_summary <- ppt_long_data |>
  ...
  mutate(weighted_mean_total_daily_precipitation_ukl_catchment_in = lag(
         ...
         thirty_d_trailing_sum_precip = rollsum(weighted_mean_total_daily_precipitation_ukl_catchment_in, k = 30, fill=NA, align = "right")
         ) |>
