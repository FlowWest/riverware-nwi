...
three_mta_norm_values <- read_csv(here::here("data/climate_indicies_data/normalizedsheet.csv"))
```
Outputs: 
...
clean_pdo_df_date <- clean_pdo_df |> 
  mutate(date = as.Date(paste0(index, "-01"))) |> glimpse()

max_date <- clean_pdo_df_date |> 
  pull(date) |> 
  max() |>            
  `%m+%`(months(1))
...
full_normalized_df <- clean_n34_df|>
  ...
  full_join(clean_pdo_df_date, by = c("month", "index", "Year","date")) |>
  left_join(normalizing_values, by = "month")|>
  left_join(three_mta_norm_values, by="month") |> #TODO ADD ROW
  add_row(date=max_date) |>
  rowwise()|>
  ...
  rowwise() |>
  mutate(
    normalized_3mta_normalized_PDO = normalize(three_mta_normalized_PDO, min_3mta_norm_PDO, max_3mta_norm_PDO),
    normalized_3mta_normalized_PDO_CN34 = normalize(three_mta_normalized_PDO_CN34, min_3mta_norm_PDO_CN34, max_3mta_norm_PDO_CN34)) |>  
  ungroup() |>  
  mutate(
...
full_normalized_df[nrow(full_normalized_df),]$normalized_3mta_normalized_PDO_CN34_lag_0 <- if_else(is.na(full_normalized_df[nrow(full_normalized_df)-1,]$normalized_3mta_normalized_PDO_CN34), full_normalized_df[nrow(full_normalized_df)-1,]$normalized_3mta_normalized_PDO_CN34_lag_0, full_normalized_df[nrow(full_normalized_df)-1,]$normalized_3mta_normalized_PDO_CN34)
