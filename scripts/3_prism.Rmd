...
* PPT Download (data/prism_data/prism_ppt_download.csv)
* PPT, Tmean, Tmax joined download (data/prism_data/prism_download.csv)
...
TODOs:
* Decide on start date and end date for Prism time series data. Default start date is 15 days before current date. Default end date is current date.
* Decide on download location director - downloading historical files could add up so make sure there's enough disk space.
* If downloading historic data, only need PPT.

Note: also first time generation will need historic data to generate the values for `trailing_sum_31_1095_d_precip_in`

```{r}
prism_start_date = Sys.Date() - 15
prism_end_date = Sys.Date()
...
## Loop through elements, download zipped nc files, unzip nc files and parsel for the interested location 
```{r}
...
write_csv(ppt_long_data, here::here("data/prism_data/prism_ppt_download.csv"))
...
write_csv(full_download_data, here::here("data/prism_data/prism_download.csv"))
```

## Generate data summaries, normalized values 
```{r}
