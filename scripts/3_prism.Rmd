...
```
```{r}
# ppt_archival <- read_csv(here::here("data/prism_data/archive_prism.csv")) |>
#   pivot_longer(!Date, names_to = "name", values_to = "ppt") |>
#   mutate(date = as.Date(Date, format = "%m/%d/%Y")) |>
#   dplyr::select(-("Date")) |> glimpse()
```
```{r}
ppt_hist_early <- read_csv(here::here("data/prism_data/archival_hist_prism_ppt.csv")) |>
...
```
```{r}
ppt_early <- read_csv(here::here("data/prism_data/archive_prism.csv")) |> 
  pivot_longer(!Date, names_to = "name", values_to = "ppt") |> 
  mutate(date = as.Date(Date, format = "%m/%d/%Y")) |> 
  dplyr::select(-("Date")) |> 
...
ppt_long_data_provisional <- ppt_long_recent_data |>
  ...
  summarise(mean = mean(ppt)) |>
pivot_wider(names_from = "group", values_from ="mean") 
ppt_long_data_provisional <- bind_rows(ppt_hist_early, ppt_long_data_provisional) |> 
  arrange(date)
