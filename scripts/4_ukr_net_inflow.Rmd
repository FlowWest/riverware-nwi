...
combined_net_inflow_smooth$ukl_net_inflow_smoothed[1] = combined_net_inflow_smooth$ukl_net_inflow_smoothed[2]
...
l1_hist <- read_csv(here::here("data/ukr_net_inflow/l1_hist.csv")) |> 
  mutate(month_day_index = as.Date(month_day_index, format="%m-%d"),
         month = month(month_day_index),
         day = day(month_day_index)) |>
  dplyr::select(-c("month_day_index")) |> glimpse()


l1_taf <- l1_taf|> 
  mutate(
    thirty_d_trailing_sum_uklni_taf = lag(zoo::rollsum(I1_taf, k = 30, fill=NA, align = "right"), n=1)) |> glimpse()
...
net_inflow_export <- combined_net_inflow_smooth |>
  ...
  ungroup() |> 
  left_join(l1_hist, by=c("month", "day")) |> 
  mutate(
      agency_barnes_reconnected = ifelse(date > "2025-01-01", 1, 0),
      preceding_alb_connection_index = ifelse(date>"2024-12-2" & date <"2025-01-2", 1, 
                                              0),
      avg_l1_hist_minus_l1_taf = ifelse(preceding_alb_connection_index==1,
                                        avg_l1_hist_minus_l1_taf, 0),
      ukl_net_inflow_smoothed = ifelse(preceding_alb_connection_index==1, 
                                       ukl_net_inflow_smoothed - 
                                       avg_l1_hist_minus_l1_taf, 
                                       ukl_net_inflow_smoothed),
      ukl_net_inflow_30_d_trailing_sum_taf = lag(zoo::rollsum(ukl_net_inflow_smoothed, k = 30, fill=NA, align = "right"))) |> 
  left_join(simulated_net_inflow_30_d_trailing_sum, by=c("day_of_water_year")) |>
  rowwise() |> 
  mutate(
         normalized_30_d_trailing_sum_UKLNI = ifelse(agency_barnes_reconnected == 0,
normalize(ukl_net_inflow_30_d_trailing_sum_taf,observed_min_thirty_d_trailing_sum_uklni_taf,observed_max_thirty_d_trailing_sum_uklni_taf), normalize(ukl_net_inflow_30_d_trailing_sum_taf, simulated_min_thirty_d_trailing_sum_uklni_taf, simulated_max_thirty_d_trailing_sum_uklni_taf))
         ) |>
  ungroup() |>
  mutate(
