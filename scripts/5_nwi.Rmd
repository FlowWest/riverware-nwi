...
prism_data <- read_csv(here::here("data/prism_data/archival_prism_ppt_summary_export.csv")) |> glimpse()
...
prism_data_edit <-prism_data |>
  ...
  mutate(month = as.integer(format(date, "%m")),
         ...
         leap_year_index = if_else(leap_year(water_year), 1, 0)
         )
...
nwi <- prism_full |> 
  ...
  mutate(day_of_water_year = as.integer(day_of_water_year),
         normalized_weighted_avg_swe = if_else(day_of_water_year < 16 | (month >= 7 & month <=9), NA, normalized_weighted_avg_swe)
         ) |>
  ...
  mutate(weight_for_30_d_trailing_sum_UKLNI = day_of_water_year*wi_slope + wi_intercept,
         ...
         climate_index_lag_0 = ifelse(climate_index_used == "3 mta PDO_CN34", normalized_3mta_normalized_PDO_CN34_lag_0, normalized_3mta_normalized_PDO_lag_0),
         wetness_index_v20 = 
           ...
           replace_na(normalized_weighted_avg_swe, 0)*weight_for_normalized_weighted_avg_swe +
           climate_index_lag_0*weight_for_climate_index
         ) |>
  left_join(nwi_min_max, by=c("day_of_water_year")) |>
  rowwise() |>
  mutate(normalized_wetness_index_v20 = normalize(wetness_index_v20, min_nwi_v20, max_nwi_v20)) |>
  ungroup() |>
  mutate(
  ...
  mutate(trailing_2_week_mean_nwi_v20 = zoo::rollmean(normalized_wetness_index_v20, k=14, fill=NA, align = "right"))
