...
swe_summary <- full_swe_df |> 
  ...
  rowwise()|>
  mutate(normalized_average_swe = normalize(weighted_average_swe, 
                                                  min_weighted_average_swe_in, 
                                                  max_weighted_average_swe_in))|>
  ungroup() |>
  group_by(water_year) |> 
  mutate(
    swe_may15 = normalized_average_swe[
      month(Date) == 5 & day(Date) == 15
    ][1],
    normalized_average_swe = case_when(
      day_of_water_year == 366 ~ lag(normalized_average_swe),
      Date >= ymd(paste0(water_year - 1, "-05-16")) &
      Date <= ymd(paste0(water_year - 1, "-06-30")) ~ swe_may15,
      TRUE ~ normalized_average_swe
    )
  ) |> 
  mutate(
    ...
    oct_16_nov_30_linear_weight = day_after_oct_15 * (1/46),
    ...
    may_16_jun_30_linear_weight = day_after_may_15_desc * (1/46),
    normalized_weighted_avg_swe = case_when(!is.na(day_after_oct_15) ~ normalized_average_swe * oct_16_nov_30_linear_weight,
                                            !is.na(day_after_may_15_desc) ~ swe_may15 * may_16_jun_30_linear_weight,
