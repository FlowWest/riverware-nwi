...
l1_taf <- l1_taf|> 
  mutate(thirty_d_trailing_sum_uklni_taf = lag(round(rollsum(I1_taf, k = 30, fill=NA, align = "right"), 2), n=1)) |> glimpse()
...
combined_net_inflow_smooth_roll_sum <- combined_net_inflow_smooth |>
  mutate(
  ...
  ungroup() |> 
  mutate(
    ukl_net_inflow_30_d_trailing_sum_taf = lag(rollsum(ukl_net_inflow_smoothed, k = 30, fill=NA, align = "right"), 1)) |>View() 
  left_join(simulated_net_inflow_30_d_trailing_sum, by=c("day_of_water_year")) |>
  rowwise() |> 
  mutate(
         normalized_30_d_trailing_sum = round(normalize(ukl_net_inflow_30_d_trailing_sum_taf,observed_min_thirty_d_trailing_sum_uklni_taf,observed_max_thirty_d_trailing_sum_uklni_taf
                                                ), 2) 
         ) |> View()
# any(format(combined_net_inflow_smooth_roll_sum$date, "%m-%d") == "02-29")
